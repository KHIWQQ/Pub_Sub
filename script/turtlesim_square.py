...
class Turtlebot:
    def __init__(self):
        ...
        self.vel_pub = rospy.Publisher("tertle1/cmd_vel", Twist, queue_size=10)
        self.current = Pose()
    ...
    def set_start(self):
        ...
        rospy.loginfo(f"Set start ponit at x = {self.start_pose.x} y = {self.start_pose.y} the = {self.start_pose.theta}")

    def distance_error(self, distance_goal):    
        self.rate.sleep()
        distance = math.sqrt(math.pow(self.current.x - self.start_pose.x, 2)+ math.pow(self.currenty - self.start_pose.y, 2))
        dist_error = distance_goal - distance
        rospy.loginfo("distance = {dist_error}")
        return dist_error
    
    def move(self, distance):
        self.set_start()
        rospy.loginfo("Start moving {distance} unit")
    ...
    def angular_error(self, angular):
        self.rate.sleep()
        self.set_start()
